...
namespace seal
{
        ...
        MemoryPoolHandle pool) const
    {
        ...
        // --- Phase 2: Encryption ---
        // The existing encrypt_internal function
        encrypt_internal(encoded_plain, true, false, destination, pool);
    }
        ...
        Ciphertext &destination, MemoryPoolHandle pool) const
    {
        ...
        // --- Phase 2: Encryption ---
        // Pass 'true' for is_asymmetric. 'false' for save_seed.
        encrypt_internal(encoded_plain, true, false, destination, pool);
    }
    ...
    void Encryptor::encode_and_encrypt_symmetric_ckks(
        const std::vector<double> &values, parms_id_type parms_id, double scale, Ciphertext &destination,
        MemoryPoolHandle pool, bool seeded) const
    {
        // --- Phase 1: Encoding ---
        // First, use the public CKKSEncoder to encode the data into a plaintext.
        CKKSEncoder encoder(context_);
        Plaintext encoded_plain;
        encoder.encode(values, parms_id, scale, encoded_plain, pool);

        // --- Phase 2: Symmetric Encryption ---
        encrypt_internal(encoded_plain, false, seeded, destination, pool);
    }

    // Implementation for std::vector<std::complex<double>> with symmetric CKKS encryption
    void Encryptor::encode_and_encrypt_symmetric_ckks(
        const std::vector<std::complex<double>> &values, parms_id_type parms_id, double scale,
        Ciphertext &destination, MemoryPoolHandle pool, bool seeded) const
    {
        // --- Phase 1: Encoding ---
        // First, use the public CKKSEncoder to encode the data into a plaintext.
        CKKSEncoder encoder(context_);
