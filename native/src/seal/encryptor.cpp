...
namespace seal
{
    ...
    /////////////////////////////////////////////
    // COMBINED ENCODE AND ENCRYPT FOR CKKS
    /////////////////////////////////////////////

    // Implementation for std::vector<double>
    void Encryptor::encode_and_encrypt_ckks(
        const std::vector<double> &values, parms_id_type parms_id, double scale, Ciphertext &destination,
        MemoryPoolHandle pool) const
    {
        // --- Phase 1: Encoding ---
        // Verify parameters for this function's direct responsibilities
        auto context_data_ptr = context_.get_context_data(parms_id);
        if (!context_data_ptr)
        {
            throw std::invalid_argument("parms_id is not valid for encryption parameters");
        }
        // CKKSEncoder::encode will perform its own detailed validation of values, scale, etc.

        CKKSEncoder encoder(context_); // Create CKKSEncoder instance
        Plaintext encoded_plain(pool);

        // Use the public CKKSEncoder::encode method for std::vector<double>
        encoder.encode(values, parms_id, scale, encoded_plain, pool);
        // encoded_plain is now correctly encoded and in NTT form.
        ...
        // Encrypt zero first. For CKKS, this uses the parms_id of the plaintext.
        // true for is_asymmetric, false for save_seed
    ...
    // Implementation for std::vector<std::complex<double>>
    void Encryptor::encode_and_encrypt_ckks(
        ...
        Ciphertext &destination, MemoryPoolHandle pool) const
    {
        ...
        // Use the public CKKSEncoder::encode method for std::vector<std::complex<double>>
        encoder.encode(values, parms_id, scale, encoded_plain, pool);
        // encoded_plain is now correctly encoded and in NTT form.
